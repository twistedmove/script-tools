...
function score_report() {
    ...
    if [[ $stage -le $1 && $score == true ]]; then
        steps/score_kaldi.sh --cmd $cmd_train $dev_dir $gph_dir $dec_dir || exit 1
...
if [[ $stage -le 2 && $train == true ]]; then
    ...
    steps/train_mono.sh --cmd $cmd_train --nj 10 data/tr100k data/lang $exp_dir || exit 1
    utils/mkgraph.sh data/lang_test $exp_dir $gph_dir || exit 1
    steps/decode.sh --cmd $cmd_train --config conf/decode.config --nj 10 $gph_dir $dev_dir $dec_dir || exit 1
fi
...
exp_dir="exp/delta_delta"
gph_dir="$exp_dir/graph"
dec_dir="$exp_dir/decode"
mdl_dir="exp/delta"
ali_dir="exp/delta_align"

if [[ $stage -le 4 && $train == true ]]; then
    echo "4. DELTA+DELTA TRAINING"
    steps/align_si.sh --cmd $cmd_train --nj 10 data/train data/lang $mdl_dir $ali_dir || exit 1
    steps/train_deltas.sh --cmd $cmd_train 2500 20000 data/train data/lang $ali_dir $exp_dir || exit 1 
    utils/mkgraph.sh data/lang_test $exp_dir $gph_dir || exit 1
    steps/decode.sh --cmd $cmd_train --config conf/decode.config --nj 10 $gph_dir $dev_dir $dec_dir || exit 1
...
mdl_dir="exp/delta_delta"
# current output alignments direction
ali_dir="exp/delta_delta_align"
...
if [[ $stage -le 6 && $train == true ]]; then
    ...
    utils/mkgraph.sh data/lang_test $exp_dir $gph_dir || exit 1
    steps/decode_fmllr.sh --cmd $cmd_train --config conf/decode.config --nj 10 $gph_dir $dev_dir $dec_dir || exit 1
fi
...
if [[ $stage -le 8 && $train == true ]]; then
    ...
    utils/mkgraph.sh data/lang_test $exp_dir $gph_dir || exit 1
    steps/decode_sgmm2.sh --cmd $cmd_train --nj 10 --config conf/decode.config --transform-dir exp/sat1/decode $gph_dir $dev_dir $dec_dir || exit 1;
fi
